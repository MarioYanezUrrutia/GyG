...
const LayoutNavBar = ({ logo = 'https://via.placeholder.com/150x50/0a0059/FFFFFF?text=LOGO', navItems = [], onLoginClick = () => {}, onCartClick = () => {}, onCategoryClick = () => {}, onLogoClick = () => {}, onSearch = () => {}, categoriesData = [] }) => {
  const [hoveredCategory, setHoveredCategory] = useState(null);

  const getCategoryProducts = (categoryName) => {
    const category = categoriesData.find(cat => cat.name === categoryName);
    return category ? category.products.slice(0, 4) : []; // Mostrar solo 4 productos
  };
                  ...
                  <div className="absolute left-1/2 transform -translate-x-1/2 mt-2 p-4 bg-white rounded-lg shadow-xl w-[80vw] max-w-4xl z-20 border border-gray-200 animate-fade-in-down">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-96 overflow-y-auto">
                      {getCategoryProducts(item).map((product) => (
                        <div key={product.id} className="bg-white rounded-lg p-3 shadow-md border border-gray-100">
                          <img 
                            src={product.image} 
                            alt={product.name}
                            className="w-full h-32 object-cover rounded-md mb-2"
                          />
                          <h4 className="text-sm font-semibold text-[#0a0059] mb-1">{product.name}</h4>
                          <p className="text-xs text-gray-600 mb-2">{product.price}</p>
                          <button 
                            onClick={() => onCategoryClick(item)}
                            className="w-full py-1 bg-gradient-to-br from-[#0a0059] to-[#f400e3] text-white text-xs rounded-md hover:from-[#f400e3] hover:to-[#0a0059] transition-all duration-300"
                          >
                            Ver categoría
                          </button>
                        </div>
